...
For email:
```bash
python scripts/apply.py apply <target> --name "<name>" --email "<email>" --phone "<phone>" --resume <path>
```

For Greenhouse (requires explicit confirmation):
```bash
python scripts/apply.py apply <target> --name "<name>" --email "<email>" --phone "<phone>" --resume <path> --consent --confirm
```
