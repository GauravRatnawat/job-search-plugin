...
```bash
python scripts/cache.py use <name>
```
...
```bash
python scripts/cache.py save profile '<json>' -p <persona-slug>
```
...
```bash
python scripts/cache.py save search_strategy '<JSON of search queries>'
```
...
```bash
python scripts/cache.py save search_results '<JSON array of job listings>'
```
...
```bash
python scripts/cache.py save scored_jobs '<JSON array of scored jobs>'
```
