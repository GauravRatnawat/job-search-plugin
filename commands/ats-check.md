...
2. Read `.cache/<persona>/scored_jobs.json` → find job by position in list (1-indexed)
3. Get `url` field from that job
4. If URL looks like a specific posting (not a generic board link — not `linkedin.com/jobs`, `indeed.com/jobs`, `greenhouse.io/jobs` without posting ID):
   - Use WebSearch: `"<company> <title> job description"` to find the actual JD text
   - If found: extract JD text and proceed
   - If not found or generic result: ask user "I couldn't retrieve the full job description. Please paste it here."
5. If URL IS a generic board link: skip URL and ask user to paste JD directly
